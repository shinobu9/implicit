...
def least_squares_cg_proj(Cui, X, Y, regularization, A, d_u, num_threads=0, cg_steps=3):
    ...
    return _least_squares_cg_proj(Cui.indptr, Cui.indices, Cui.data.astype('float32'),
                             X, Y, regularization, A, d_u, num_threads, cg_steps)
...
@cython.boundscheck(False)
def _least_squares_cg_proj(integral[:] indptr, integral[:] indices, float[:] data,
    ...
    cdef floating[:, :] Yu = np.zeroes((Y.shape[0],Y.shape[1]), dtype=dtype)
    cdef floating[:, :] Ireg = np.eye(N, dtype=dtype) * regularization
    cdef floating[:, :] YtY = np.dot(np.transpose(Y), Y) + Ireg
    ...
    try:
        for u in range(users, schedule='dynamic', chunksize=8):
            ...
            # Recalculate YtY by computing dot product of A[d_u[u]] and Y
            Yu = np.dot(A[d_u[u]], Y)
            YtY = np.dot(np.transpose(Yu), Yu) + Ireg # assuming A is precomputed for each user
            ...
            for index in range(indptr[u], indptr[u + 1]):
                ...
                temp = temp - (confidence - 1) * dot(&N, &Yu[i, 0], &one, x, &one)
                axpy(&N, &temp, &Yu[i, 0], &one, r, &one)
            ...
            for it in range(cg_steps):
                ...
                for index in range(indptr[u], indptr[u + 1]):
                    ...
                    temp = (confidence - 1) * dot(&N, &Yu[i, 0], &one, p, &one)
                    axpy(&N, &temp, &Yu[i, 0], &one, Ap, &one)
